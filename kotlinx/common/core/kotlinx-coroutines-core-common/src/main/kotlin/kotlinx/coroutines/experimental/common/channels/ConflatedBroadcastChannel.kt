...
import kotlinx.coroutines.experimental.common.expect.arraycopy
import kotlinx.coroutines.experimental.common.internal.Symbol
...
public class ConflatedBroadcastChannel<E>() : BroadcastChannel<E> {
    ...
    private val _state = AtomicReference<Any>(INITIAL_STATE) // State | Closed
    private val _updating = AtomicReference<Int>(0)
    ...
    private class State<E>(
            val value: Any?, // UNDEFINED | E
            val subscribers: Array<Subscriber<E>>?
    )

    private class Closed(val closeCause: Throwable?) {
...
//        _state.loop { state ->
    ...
    override val isClosedForSend: Boolean get() = _state.get() is Closed
    ...
    private fun removeSubscriber(list: Array<Subscriber<E>>, subscriber: Subscriber<E>): Array<Subscriber<E>>? {
        ...
        arraycopy<Subscriber<E>>(list, 0, update, 0, i)
        arraycopy<Subscriber<E>>(list, i + 1, update, i, n - i - 1)
    ...
    private fun offerInternal(element: E): Closed? {
        ...
        try {
            ...
            _updating.set(0) // reset the updating flag to zero even when something goes wrong
        }
    }
