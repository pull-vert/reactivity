...
import kotlinx.coroutines.experimental.common.expect.CancellableContinuationImpl
 ...
 * ```
 */
public interface CancellableContinuationCommon<in T> : Continuation<T>, Job {
    /**
...
public inline suspend fun <T> suspendCancellableCoroutine(
    ...
    suspendCoroutineOrReturn { cont ->
        val cancellable = CancellableContinuationImpl(cont, MODE_CANCELLABLE)
...
public inline suspend fun <T> suspendAtomicCancellableCoroutine(
    ...
    suspendCoroutineOrReturn { cont ->
        val cancellable = CancellableContinuationImpl(cont, MODE_ATOMIC_DEFAULT)
        if (!holdCancellability) cancellable.initCancellability()
        block(cancellable)
        cancellable.getResult()
    }
...
abstract class AbstractCancellableContinuation<in T>(
