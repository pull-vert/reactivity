...
import kotlinx.coroutines.experimental.common.expect.*
import kotlinx.coroutines.experimental.common.internal.LockFreeLinkedListHead
import kotlinx.coroutines.experimental.common.internal.LockFreeLinkedListNode
import kotlinx.coroutines.experimental.common.internal.OpDescriptor
import kotlinx.coroutines.experimental.common.internal.unwrap
import kotlinx.coroutines.experimental.common.intrinsics.startCoroutineUndispatched
import kotlinx.coroutines.experimental.common.selects.SelectClause0
import kotlinx.coroutines.experimental.common.selects.SelectClause1
import kotlinx.coroutines.experimental.common.selects.SelectInstance
import kotlinx.coroutines.experimental.common.selects.select
 ...
 */
public typealias CancellationException = kotlinx.coroutines.experimental.common.expect.CancellationException
...
public class JobCancellationException(
    ...
    init { if (cause != null) initCauseThrowable(cause) }
...
public open class JobSupport(active: Boolean) : Job, SelectClause0, SelectClause1<Any?> {
    ...
    private val _state = AtomicReference<Any?>(if (active) EmptyActive else EmptyNew)
    ...
    protected val state: Any? get() {
        // _state.loop { state ->
        while (true) {
            val state = _state.get() // helper loop on state (complete in-progress atomic operations)
     ...
     * @suppress **This is unstable API and it is subject to change.**
     */
    protected open fun nameString(): String? = this::class.simpleName
    ...
    class NodeList(
        ...
        private val _active = AtomicReference(if (active) 1 else 0)

        override val isActive: Boolean get() = _active.get() != 0
        override val list: NodeList get() = this

        fun tryMakeActive(): Int {
            if (_active.get() != 0) return FALSE
    ...
    private suspend fun awaitSuspend(): Any? = suspendCancellableCoroutine { cont ->
        cont.disposeOnCompletion(invokeOnCompletion(false, object : CompletionHandler {
            override operator fun invoke(reason: Throwable?) {
...
abstract class JobNode<out J : Job>(
    val job: J
...
private class InvokeOnCompletion(
    ...
    //"InvokeOnCompletion[${handler::class.qualifiedName}@${Integer.toHexString(System.identityHashCode(handler))}]"
}
...
private class InvokeOnCancellation(
    ...
    private val _invoked = AtomicReference(0)
    override fun invoke(reason: Throwable?) {
