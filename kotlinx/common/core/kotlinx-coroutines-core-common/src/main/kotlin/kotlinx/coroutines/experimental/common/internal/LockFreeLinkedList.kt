...
package kotlinx.coroutines.experimental.common.internal

import kotlinx.coroutines.experimental.common.expect.AtomicReference
...
open class LockFreeLinkedListNode {
    private val _next = AtomicReference<Any>(this) // Node | Removed | OpDescriptor
    private val _prev = AtomicReference<Any>(this) // Node | Removed
    private val _removedRef = AtomicReference<Removed?>(null) // lazily cached removed ref to this

    private fun removed(): Removed =
        _removedRef.get() ?: Removed(this).also { _removedRef.lazySet(it) }
    ...
    public val isFresh: Boolean get() = _next.get() === this
    ...
    public open fun describeRemove() : AtomicDesc? {
        ...
        return object : AbstractAtomicDesc() {
            private val _originalNext = AtomicReference<Node?>(null)
            override val affectedNode: Node? get() = this@LockFreeLinkedListNode
            override val originalNext get() = _originalNext.get()
    ...
    public open class AddLastDesc<out T : Node>(
        ...
        init {
            ...
            check(node._next.get() === node && node._prev.get() === node)
        }

        final override fun takeAffectedNode(op: OpDescriptor): Node {
            while (true) {
                val prev = queue._prev.get() as Node // this sentinel node is never removed
                val next = prev._next.get()
        ...
        private val _affectedNode = AtomicReference<Node?>(null)
        final override val affectedNode: Node? get() = _affectedNode.get()
    ...
    public open class RemoveFirstDesc<T>(
        ...
        final override val affectedNode: Node? get() = _affectedNode.get()
        final override val originalNext: Node? get() = _originalNext.get()
    ...
    private fun finishRemove(next: Node) {
        helpDelete()
        next.correctPrev(_prev.get().unwrap(), null)
    }
    ...
    internal fun helpDelete() {
        ...
        var next: Node = (this._next.get() as Removed).ref
        while (true) {
    ...
    private fun correctPrev(_prev: Node, op: OpDescriptor?): Node? {
        ...
        while (true) {
            ...
            if (this._prev.compareAndSet(oldPrev, prev)) {
                if (prev._prev.get() !is Removed) return null // finish only if prev was not concurrently removed
            }
        }
    }

    internal fun validateNode(prev: Node, next: Node) {
        check(prev === this._prev.get())
        check(next === this._next.get())
    }

    override fun toString(): String = "${this::class.simpleName}" // @${Int.toHexString(System.identityHashCode(this))}
}
