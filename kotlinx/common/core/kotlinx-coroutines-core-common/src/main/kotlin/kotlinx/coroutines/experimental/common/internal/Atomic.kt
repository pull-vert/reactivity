 ...
 * @suppress **This is unstable API and it is subject to change.**
 */
abstract class AtomicOp<in T> : OpDescriptor() {
    private val _consensus = AtomicReference<Any?>(NO_DECISION)

    val isDecided: Boolean get() = _consensus.get() !== NO_DECISION
    ...
    private fun decide(decision: Any?): Any? = if (tryDecide(decision)) decision else _consensus.get()
