...
package kotlinx.coroutines.experimental.common.sync
...
import kotlinx.coroutines.experimental.common.expect.CancellableContinuation
...
import kotlinx.coroutines.experimental.common.removeOnCancel
import kotlinx.coroutines.experimental.common.selects.ALREADY_SELECTED
import kotlinx.coroutines.experimental.common.selects.SelectClause2
import kotlinx.coroutines.experimental.common.selects.SelectInstance
import kotlinx.coroutines.experimental.common.suspendAtomicCancellableCoroutine
...
internal class MutexImpl(locked: Boolean) : Mutex, SelectClause2<Any?, Mutex> {
    ...
    private val _state = AtomicReference<Any?>(if (locked) EmptyLocked else EmptyUnlocked)

    // resumeNext is: RESUME_QUIESCENT | RESUME_ACTIVE | ResumeReq
    private val _resumeNext = AtomicReference<Any>(RESUME_QUIESCENT)
...
//        _state.loop { state ->
        while (true) {
            ...
            when (state) {
                ...
                is LockedQueue -> {
                    ...
                    if (state.addLastIf(waiter, { _state.get() === state })) {
    ...
    private class TryEnqueueLockDesc<R>(
        ...
        override fun onPrepare(affected: LockFreeLinkedListNode, next: LockFreeLinkedListNode): Any? {
            if (mutex._state.get() !== queue) return ENQUEUE_FAIL
...
//        _resumeNext.loop { resumeNext ->
            while (true) {
                val resumeNext = _resumeNext.get()
            when {
...
//        _state.loop { state ->
    ...
    private class UnlockOp(
        ...
        override fun perform(affected: Any?): Any? {
             ...
             */
            return if (affected._state.get() === queue) UNLOCK_FAIL else null
        }
    }
}
