...
package kotlinx.coroutines.experimental.common.selects

import kotlinx.coroutines.experimental.common.Deferred
import kotlinx.coroutines.experimental.common.DisposableHandle
import kotlinx.coroutines.experimental.common.Job
import kotlinx.coroutines.experimental.common.JobCancellationNode
import kotlinx.coroutines.experimental.common.channels.ReceiveChannel
import kotlinx.coroutines.experimental.common.channels.SendChannel
import kotlinx.coroutines.experimental.common.expect.*
import kotlinx.coroutines.experimental.common.internal.*
import kotlinx.coroutines.experimental.common.sync.Mutex
 ...
 */
public interface SelectBuilderCommon<in R> {
    /**
...
val ALREADY_SELECTED: Any = Symbol("ALREADY_SELECTED")
private val UNDECIDED: Any = Symbol("UNDECIDED")
private val RESUMED: Any = Symbol("RESUMED")

abstract class AbstractSelectBuilder<in R>(
    ...
    private val _state = AtomicReference<Any?>(this)

    // this is basically our own SafeContinuation
    private val _result = AtomicReference<Any?>(UNDECIDED)
    ...
    private inner class SelectOnCancellation(job: Job) : JobCancellationNode<Job>(job) {
        ...
        override fun toString(): String = "SelectOnCancellation[${this@AbstractSelectBuilder}]"
    }
    ...
    private inner class AtomicSelectOp(
        ...
        fun prepareIfNotSelected(): Any? {
            while (true) {
                ...
                when {
                    ...
                    state is OpDescriptor -> state.perform(this@AbstractSelectBuilder) // help
                    state === this@AbstractSelectBuilder -> {
                        if (_state.compareAndSet(this@AbstractSelectBuilder, this@AtomicSelectOp))
                            return null // success
                    }
        ...
        private fun completeSelect(failure: Any?) {
            ...
            val update = if (selectSuccess) null else this@AbstractSelectBuilder
    ...
    override fun SelectClause0.invoke(block: suspend () -> R) {
        registerSelectClause0(this@AbstractSelectBuilder, block)
    }

    override fun <Q> SelectClause1<Q>.invoke(block: suspend (Q) -> R) {
        registerSelectClause1(this@AbstractSelectBuilder, block)
    }

    override fun <P, Q> SelectClause2<P, Q>.invoke(param: P, block: suspend (Q) -> R) {
        registerSelectClause2(this@AbstractSelectBuilder, param, block)
    }
