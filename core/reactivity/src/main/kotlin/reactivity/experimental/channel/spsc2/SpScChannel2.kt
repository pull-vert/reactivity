package reactivity.experimental.channel.spsc2
...
//        println("handleFull offset=$offset full=$full")
    ...
    private suspend fun sendSuspend(fullBuffer: AtomicReferenceArray<Suspended2?>, offset: Int): Unit = suspendCoroutine { cont ->
        soFull(fullBuffer, offset, Suspended2(cont))
        // handle empty case (= suspended Consumer)
        handleEmpty()
...
//            println("receive ${value.item}")
    ...
    private suspend fun receiveSuspend(emptyBuffer: AtomicReferenceArray<Suspended2?>, offset: Int): Unit = suspendCoroutine { cont ->
        // StoreStore
        soEmpty(emptyBuffer, offset, Suspended2(cont))
...
abstract class AtomicReferenceArrayEmpty<E : Any>(capacity: Int) : SpscAtomicArrayQueueL3Pad2<E>(capacity) {
    @JvmField internal val emptyBuffer = AtomicReferenceArray<Suspended2?>(capacity)

    // todo remove if possible
    internal fun getAndUpdateEmpty(offset: Int, new: Suspended2?): Suspended2? {
        while(true) {
    ...
    protected companion object {
        @JvmStatic
        internal fun lvEmpty(emptyBuffer: AtomicReferenceArray<Suspended2?>, offset: Int) = emptyBuffer.get(offset)

        @JvmStatic
        internal fun soEmpty(emptyBuffer: AtomicReferenceArray<Suspended2?>, offset: Int, value: Suspended2?) = emptyBuffer.lazySet(offset, value)
    }
}
...
abstract class AtomicReferenceArrayFull<E : Any>(capacity: Int) : SpscAtomicArrayQueueL6Pad<E>(capacity) {
    @JvmField internal val fullBuffer = AtomicReferenceArray<Suspended2?>(capacity)

    // todo remove if possible
    internal fun getAndUpdateFull(offset: Int, new: Suspended2?): Suspended2? {
        while(true) {
    ...
    protected companion object {
        @JvmStatic
        internal fun lvFull(fullBuffer: AtomicReferenceArray<Suspended2?>, offset: Int) = fullBuffer.get(offset)

        @JvmStatic
        internal fun soFull(fullBuffer: AtomicReferenceArray<Suspended2?>, offset: Int, value: Suspended2?) = fullBuffer.lazySet(offset, value)
    }
}
