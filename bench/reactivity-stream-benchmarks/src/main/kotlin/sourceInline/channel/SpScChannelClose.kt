 ...
 */
open class SpScChannelClose<E : Any>(
        /**
         * Buffer capacity.
         */
        capacity: Int
) : SpscAtomicArrayQueueL5Pad<E>(capacity), Sink<E> {
    ...
    override fun close(cause: Throwable?) {
        throw ClosedReceiveChannelException("closed")
    }
...
//            println("Consumer : tryResumeSend -> Producer is suspended, resume")
     ...
     */
    private fun offer(item: E, producerIndex: Long): Boolean {
        val mask = this.mask
...
//        println("Producer : offer -> offer offset=$offset $item")
    ...
    private suspend fun send(item: E, prevProducerIndex: Long?) {
...
abstract class AtomicReferenceArrayQueue<E : Any>(capacity: Int) {
    @JvmField protected val buffer = AtomicReferenceArray<Element<E>?>(capacity)
    ...
    protected fun loCompareAndSetExpectedNullElement(offset: Int, value: Element<E>) = buffer.compareAndSet(offset, null, value)
